...
void CustomLog(std::string ErrMsg)
{
    MessageBoxA(0, ErrMsg.c_str(), "Error", MB_OK);
}
...
#pragma warning(default : 6387) // value is checked before function call yet warning still appears
...
#pragma warning(default : 6387) // value is checked before function call yet warning still appears
    ...
    PIMAGE_THUNK_DATA GetFunctionThunkByName(PIMAGE_IMPORT_DESCRIPTOR Dll, std::string Function, bool Original = true)
    {
        if (!Dll)
        {
            ErrorCallBack("Invalid DLL Passed To: GetFunctionThunkByName");
        }
